...
function PipelineInterface() {
  ...
  const getTaskTypes = (services) => {
    const currentAvailableTasks = new Set();
    services.map((service) => {
      currentAvailableTasks.add(service["task"]["type"]);
    });
    return Array.from(currentAvailableTasks);
  };

  useEffect(() => {
    if (services) {
      const currentAvailableTasks = getTaskTypes(services);
      setavailableTasks(currentAvailableTasks);
      setSelectedTask(currentAvailableTasks[0] as string);
    }
  }, [services]);
      ...
      <GridItem p="1rem" bg="white" gap={5}>
        ...
        <Tabs isFitted>
          ...
          <TabPanels>
            <TabPanel>
              <Text className="dview-service-description">
                The Pipeline Interface can be used to chain various services to
                build complex tasks such as Speech2Speech.
              </Text>
            </TabPanel>
          </TabPanels>
        </Tabs>
