...
async def _run_inference_s2s_asr(
    ...
    translation_request = ULCATranslationInferenceRequest(
        serviceId="ai4bharat/indictrans-fairseq-all-gpu--t4", config=request.config, input=asr_response.output
    )
    ...
    tts_request = ULCATtsInferenceRequest(serviceId=serviceId, config=request.config, input=translation_response.output)
    ...
    response = ULCAS2SInferenceResponse(
        output=translation_response.output, audio=tts_response.audio, config=tts_response.config
    )
